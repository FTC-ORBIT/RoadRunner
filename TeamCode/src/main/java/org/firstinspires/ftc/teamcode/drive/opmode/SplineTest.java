...
public class SplineTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
       ...
       //                 .splineTo(new Vector2d(0, 0), Math.toRadians(180))
        ...
        Trajectory cycle = drive.trajectoryBuilder(new Pose2d())
                .splineToLinearHeading(new Pose2d(1.28,0.176, Math.toRadians(54)), 90).build();
        Trajectory backToTheWarHouse = drive.trajectoryBuilder(cycle.end(), true)
                .splineToLinearHeading(new Pose2d(0, 0, 0), 90)
                .build();
        drive.followTrajectory(cycle);
        drive.followTrajectory(backToTheWarHouse);
    }
}
